...
import { 
  ...
  Shield, Github, Chrome, Loader2, DollarSign, PieChart, BarChart3
} from "lucide-react";
...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (status === 'authenticated' && session) {
      // Store finance token from session
      if (session.financeToken) {
        localStorage.setItem('financeToken', session.financeToken);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Use NextAuth for credentials login/signup
      ...
      if (result?.ok) {
        ...
        // The session callback will handle storing the finance token
        ...
        <div className="hidden md:block space-y-6">
          <div className="space-y-4">
            ...
            <p className="text-lg text-muted-foreground ivy-font max-w-md">
              Your intelligent finance companion for tracking income, expenses, and gaining insights into your financial health.
            </p>
          </div>
          ...
          <div className="space-y-3 mt-8">
            {[
              { icon: TrendingUp, title: "Financial Analytics", desc: "Track income, expenses, and spending patterns" },
              { icon: PieChart, title: "Category Insights", desc: "Visualize where your money goes by category" },
              { icon: BarChart3, title: "Trend Analysis", desc: "Monthly and weekly financial trends at a glance" }
            ].map((feature, idx) => (
              <div 
                key={idx}
        ...
        <Card className="border-border/40 backdrop-blur-xl bg-card/80 shadow-2xl">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* Role preference - only for signup */}
              {!isLogin && (
                <div className="space-y-2">
                  <Label htmlFor="requestedRole" className="ivy-font">Request Role</Label>
                  <select
                    id="requestedRole"
                    name="requestedRole"
                  ...
                  >
                    <option value="viewer">Viewer (instant access)</option>
                    <option value="analyst">Analyst (requires admin approval)</option>
                    <option value="admin">Admin (requires admin approval)</option>
                  </select>
                  <p className="text-xs text-muted-foreground ivy-font">
                    Analyst and Admin access are only granted after approval by an existing admin.
                  </p>
                </div>
              )}
              ...
              <p className="text-center text-sm text-muted-foreground ivy-font mt-6">
                ...
                <button
                  ...
                  onClick={() => {
                    setIsLogin(!isLogin);
                    setFieldErrors({});
                    setFormData({ email: "", password: "", confirmPassword: "", name: "", requestedRole: "viewer" });
                  }}
